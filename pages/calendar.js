  ...
  function render(root) {
    ...
    items.forEach((item) => {
      ...
      if (item.type === 'match') {
        ...
        const commentCount = window.LH3.services.commentService.listComments(match.id).length;
        wrap.appendChild(el('button', {
          className: 'btn btn-sm btn-ghost', style: { width: '100%', marginTop: '6px' },
          onClick: (e) => { e.stopPropagation(); showCommentsModal(match, manager); },
        }, ['💬 Commentaires' + (commentCount ? ' (' + commentCount + ')' : '')]));
      } else {
